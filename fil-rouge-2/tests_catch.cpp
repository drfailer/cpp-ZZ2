...
TEST_CASE("Cercle", "[Cercle]") {
   ...
   Cercle c1;
   Cercle c2(Point(1,1),1,1,1);

   REQUIRE(c1.toString() == "CERCLE (0, 0), BLEU, 0, 0, 0");
   REQUIRE(c2.toString() == "CERCLE (1, 1), BLEU, 1, 1, 1");

   c2.setRayon(4);
   REQUIRE(c2.getRayon()   == 4);
   REQUIRE(c2.toString()   == "CERCLE (1, 1), BLEU, 1, 1, 4"); // TODO: écrire toString pour Forme et
                                        // Cercle (idem pour rectangle).
   REQUIRE(c2.getLargeur() == 1);
   REQUIRE(c2.getHauteur() == 1);
...
TEST_CASE("Polymorphisme", "[Forme]") {
   ...
   REQUIRE(f1->toString() == "CERCLE (0, 0), BLEU, 0, 0, 0");
   REQUIRE(f2->toString() == "RECTANGLE (0, 0), BLEU, 0, 0");
...
TEST_CASE("Groupe", "[Groupe]") {
   Groupe * g = new Groupe;
   g->ajouterList(new Cercle);
   g->ajouterList(new Rectangle);

   REQUIRE(g->toString() == "CERCLE (0, 0), BLEU, 0, 0, 0\nRECTANGLE (0, 0), BLEU, 0, 0\n");
